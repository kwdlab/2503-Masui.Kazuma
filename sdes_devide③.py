...
#8bitを最初と最後の2bitずつ、真ん中の4bitに分けて記録するコード
...
name = r"C:\Users\81803\Desktop\myworkbook2.xlsx"
wb.save(name)

key = 0b0000000000
plaintext = 0b00000000
num = 1
...
for j in range(1024):
  ...
  for i in range(1,256):
    ...
    ee = int(e, 2)
    ff = int(f, 2)
    ws.cell(row=num, column=1, value=bb)
    ws.cell(row=num, column=2, value=cc)
    ws.cell(row=num, column=3, value=ee)
    ws.cell(row=num, column=4, value=ff)
